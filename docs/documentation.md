# Primer
NFTStorefront contract comes to life to fulfil the growing demand of non-custodial marketplace on FLOW blockchain. This document focuses on providing the detailed information about inner workings of the NFTStorefrontV2 and what high level APIs it offers. 
...
The next step is to create a listing under the newly created storefront resource or if user (repeatitive user) already holds the storefront resource then use the existing resource. Each user would have different scenarios for their listings, We cover most of them below.
