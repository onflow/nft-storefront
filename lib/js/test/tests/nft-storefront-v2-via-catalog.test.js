...
async function getExistingListingIDs(account, nftId) {
    const read_existing_listing_ids_script = fs.readFileSync(
        path.resolve(
            __dirname,
            "../../../../scripts/read_existing_listing_ids.cdc"
        ),
        { encoding: "utf8", flag: "r" }
    );
    const [scriptResult, sError] = await executeScript({
        code: read_existing_listing_ids_script,
        args: [account, nftId],
    });
    return scriptResult;
}
...
describe("NFTStorefrontV2", () => {
    ...
    test("should successfully list the same item multiple times and then purchase one of the listing that removes other duplicates programmatically", async () => {
        ...
        let existingListing = await getExistingListingIDs(
            seller_1,
            listingDetails.nftID
        );
        expect(existingListing.length).toEqual(2);
