...
const transfer_nft_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/transfer_nft.cdc"), {encoding:'utf8', flag:'r'});
...
async function getAllUniqueGhostListing(account) {
  const read_all_unique_ghost_listings = fs.readFileSync(path.resolve(__dirname, "../../../../scripts/read_all_unique_ghost_listings.cdc"), {encoding:'utf8', flag:'r'});
  const [scriptResult, sError] = await executeScript({ code: read_all_unique_ghost_listings, args: [account] });
  return scriptResult
}
...
describe("NFTStorefrontV2", ()=> {
  ...
  beforeEach(async () => {
    ...
    await deployContract({ to: viewResolver,                    name: "utility/ViewResolver"});
  ...
  test.only("should able to install the storefront manager in the account", async () => { 
    await shallPass(
      sendTransaction({
        name: "setup_account",
        args: [],
...
/*
  ...
  test("should successfully list the item and move the item from provider and use cleanupGhostListing to remove it", async() => {
    // step 1 : Setup a collection for seller_1
    await shallPass(
      sendTransaction({
        code: setup_nft_account_tx,
    ...
    [txResult_2, error_2] = await shallPass(
      sendTransaction({
        name: "sell_item",
        args: [0, "50.50", "TopShot Platform", "0.0", currentTimestamp + 500, []],
        signers: [seller_1]
      })
    ); 
    ...
    // Validate the details
    listingKeys = await getListingKeys(seller_1);
    expect(listingKeys.length).toEqual(2);

    // Check for the ghost listings, It should be zero at current time
    let ghostListings = await getAllUniqueGhostListing(seller_1);
    expect(ghostListings.length).toEqual(0);

    // Move the NFT to someone else i.e seller_2
    // Setup a collection for seller_2
    await shallPass(
      sendTransaction({
        code: setup_nft_account_tx,
        args: [],
    ...
    // Transfer the NFT from seller_1 -> seller_2
    await shallPass(
      sendTransaction({
        code: transfer_nft_tx,
        args: [seller_2, 0],
        signers: [seller_1]
      })
    );

    // Check for the ghost listings, It should be 1 after moving the NFT, Because one is duplicate listing
    ghostListings = await getAllUniqueGhostListing(seller_1);
    expect(ghostListings.length).toEqual(1);

    // Now anybody can cleanup the expired listing
    let [txResult_3, error_3] = await shallPass(
      sendTransaction({
        name: "cleanup_ghost_listing",
        args: [ghostListings[0], seller_1],
        signers: [buyer_1]  // can be called by anyone
      })
    );
    ...
    // Validate the details
    listingKeys = await getListingKeys(seller_1);
    expect(listingKeys.length).toEqual(0);
  });
  */
